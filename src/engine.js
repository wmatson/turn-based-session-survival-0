import { ACTIONS, DELTAS, DIRECTIONS, OUTRUN_RADIUS, SPAWN_RADIUS, VICTORY_TURN } from './engine/constants.js';
import { BREAKABLE_DEFINITIONS, ENEMY_DEFINITIONS, UPGRADE_DEFINITIONS, WEAPON_DEFINITIONS, getUpgrade, xpRequired } from './config.js';
import { chunkOf, distance, hashSeed, terrainAt } from './engine/terrain.js';
...
export const createGame = ({ seed = 1, permanent = {}, victoryTurn = VICTORY_TURN } = {}) => {
  ...
  const state = { seed, map: { seed }, turn: 0, victoryTurn, status: 'playing', victoryReached: false, runGold: 0, nextEntityId: 1,
    player: { position: [0, 0], facing: 'north', hp: maxHp, maxHp, xp: 0, level: 1, pickupRange: 1 + (permanent.startingMagnet || 0), regenerationLevel: 0, upgrades: [],
      weapons: [{ id: 'knife', type: 'knife', ...WEAPON_DEFINITIONS.knife, damage: WEAPON_DEFINITIONS.knife.damage + (permanent.sharpStart || 0) }] },
    enemies: [], breakables: [], pickups: [], pendingUpgradeChoices: [] };
  for (let cx = -2; cx <= 2; cx++) for (let cy = -2; cy <= 2; cy++) for (let trigger = 0; trigger < 2; trigger++) {
    const h = hashSeed(seed, cx, cy, 91 + trigger); if (h % 4 !== 0) continue;
...
const fireWeapon = (state, weapon, events, rng) => {
  ...
  if (targeting === 'line') { for (let i = 1; i <= weapon.range; i++) cells.push(pos(origin, state.player.facing, i)); targets = state.enemies.filter(e => cells.some(c => key(c) === key(e.position))); }
  if (targeting === 'first-in-line') { for (let i = 1; i <= weapon.range; i++) { const c = pos(origin, state.player.facing, i); cells.push(c); const hit = state.enemies.find(e => key(e.position) === key(c)); if (hit) { targets = [hit]; break; } } }
  if (targeting === 'adjacent') { for (const direction of DIRECTIONS) cells.push(pos(origin, direction)); targets = state.enemies.filter(e => cells.some(c => key(c) === key(e.position))); }
  if (targeting === 'wide-line') { const forward = pos(origin, state.player.facing, weapon.range); const lateral = state.player.facing === 'north' || state.player.facing === 'south' ? 'east' : 'north'; cells.push(pos(forward, lateral, -1), forward, pos(forward, lateral, 1)); targets = state.enemies.filter(e => cells.some(c => key(c) === key(e.position))); }
  events.push(event('weapon-fired', { weapon: weapon.type, origin, direction: state.player.facing, cells }));
  for (const target of [...targets]) if (state.enemies.some(e => e.id === target.id)) damageEnemy(state, target, weapon.damage, events, rng);
  if (damagesObjects) for (const object of [...state.breakables]) if (cells.some(c => key(c) === key(object.position))) damageObject(state, object, weapon.damage, events, rng);
};
...
const collectPickups = (state, events) => { const remaining = []; for (const pickup of state.pickups) { if (distance(state.player.position, pickup.position) > state.player.pickupRange) { remaining.push(pickup); continue; } events.push(event('pickup-collected', { pickupType: pickup.type, value: pickup.value, position: [...pickup.position] })); if (pickup.type === 'xp') { state.player.xp += pickup.value; events.push(event('xp-gained', { amount: pickup.value })); } if (pickup.type === 'gold') state.runGold += pickup.value; if (pickup.type === 'health') { const before = state.player.hp; state.player.hp = Math.min(state.player.maxHp, state.player.hp + pickup.value); if (state.player.hp !== before) events.push(event('player-healed', { amount: state.player.hp - before })); } } state.pickups = remaining; };
const offerLevelUps = (state, rng, events) => { while (state.player.xp >= xpRequired(state.player.level)) { state.player.xp -= xpRequired(state.player.level); state.player.level += 1; events.push(event('level-gained', { level: state.player.level })); const eligible = UPGRADE_DEFINITIONS.filter(u => u.eligibility(state) && (u.stackable || !state.player.upgrades.includes(u.id))); const pool = [...eligible], choices = []; while (choices.length < Math.min(3, pool.length)) { const index = rng.nextInt(pool.length); choices.push(pool.splice(index, 1)[0].id); } state.pendingUpgradeChoices = choices; state.status = 'level-up'; events.push(event('upgrade-options-generated', { choices: [...choices] })); break; } };
export const chooseUpgrade = (input, id) => { const state = clone(input); if (state.status !== 'level-up' || !state.pendingUpgradeChoices.includes(id)) return state; const upgrade = getUpgrade(id); upgrade.apply(state); state.player.upgrades.push(id); state.pendingUpgradeChoices = []; state.status = 'playing'; return state; };
