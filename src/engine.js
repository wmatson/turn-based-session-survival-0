...
export const createGame = ({ seed = 1, permanent = {}, victoryTurn = VICTORY_TURN } = {}) => {
  ...
  for (let cx = -2; cx <= 2; cx++) for (let cy = -2; cy <= 2; cy++) {
    const h = hashSeed(seed, cx, cy, 91); if (h % 4 !== 0) continue;
    const candidate = [cx * 20 + 4 + h % 11, cy * 20 + 4 + Math.floor(h / 17) % 11];
    if (key(candidate) !== '0,0' && terrainAt(state.map, ...candidate) === 'floor') state.breakables.push({ id: state.nextEntityId++, type: 'jar', position: candidate, hp: BREAKABLE_DEFINITIONS.jar.hp });
  }
  return state;
};
