...
export const renderGame = (svg, state, debug = false, effects = []) => {
  ...
  for (const pickup of state.pickups) if (inViewport(pickup.position,state.player.position)) { const [x,y]=at(pickup.position); if(pickup.type==='xp') view.append(svgEl('polygon',{points:`${x+6},${y+2} ${x+10},${y+6} ${x+6},${y+10} ${x+2},${y+6}`,fill:'#62d6ff'})); else if (pickup.type === 'health') view.append(svgEl('path',{d:'M4 0h4v4h4v4H8v4H4v-4H0V4h4z',transform:`translate(${x},${y})`,fill:'#67e58b'})); else view.append(svgEl('circle',{cx:x+6,cy:y+6,r:4,fill:'#ffd166',stroke:'#fff0a8','stroke-width':1})); }
  ...
  for (const enemy of state.enemies) { if(!inViewport(enemy.position,state.player.position)) continue; const [x,y]=at(enemy.position); const definition = ENEMY_DEFINITIONS[enemy.type] ?? ENEMY_DEFINITIONS['red-square']; const shape = definition.shape === 'circle' ? svgEl('circle',{cx:x+6,cy:y+6,r:5,fill:definition.color,...movementAttrs(enemy.id)}) : svgEl('rect',{x:x+1,y:y+1,width:10,height:10,fill:definition.color,rx:2,...movementAttrs(enemy.id)}); view.append(shape); if(debug){const t=svgEl('text',{x:x+1,y:y+9,fill:'white','font-size':5});t.textContent=enemy.id;view.append(t);} }
  const [x,y]=at(state.player.position); const player=svgEl('circle',{cx:x+6,cy:y+6,r:4.5,fill:'#f4f7ff',stroke:'#7c5cff','stroke-width':2,...movementAttrs('player')}); view.append(player); const [dx,dy] = ({north:[0,-3],south:[0,3],east:[3,0],west:[-3,0]})[state.player.facing]; view.append(svgEl('line',{x1:x+6,y1:y+6,x2:x+6+dx,y2:y+6+dy,stroke:'#ffcf5a','stroke-width':2}));
