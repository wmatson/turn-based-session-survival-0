...
export const renderGame = (svg, state, debug = false, effects = []) => {
  ...
  for (const enemy of state.enemies) { if(!inViewport(enemy.position,state.player.position)) continue; const [x,y]=at(enemy.position); view.append(svgEl('rect',{x:x+1,y:y+1,width:10,height:10,fill:'#e55252',rx:2})); if(debug){const t=svgEl('text',{x:x+1,y:y+9,fill:'white','font-size':5});t.textContent=enemy.id;view.append(t);} }
  ...
  for (const e of effects) { if (e.type !== 'weapon-fired') continue; const pts=e.cells.map(c=>at(c)); for(const [ex,ey] of pts) view.append(svgEl('polygon',{points:weaponPoints(ex,ey,e.direction),fill:e.weapon==='knife'?'#ffd166':'#8be9fd',opacity:.85,class:'weapon-effect'})); }
};
