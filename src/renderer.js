...
export const renderGame = (svg, state, debug = false, effects = []) => {
  ...
  for (const enemy of state.enemies) { if(!inViewport(enemy.position,state.player.position)) continue; const [x,y]=at(enemy.position); const shape = enemy.type === 'green-circle' ? svgEl('circle',{cx:x+6,cy:y+6,r:5,fill:'#62d47e'}) : svgEl('rect',{x:x+1,y:y+1,width:10,height:10,fill:enemy.type === 'blue-square' ? '#4f8cff' : '#e55252',rx:2}); view.append(shape); if(debug){const t=svgEl('text',{x:x+1,y:y+9,fill:'white','font-size':5});t.textContent=enemy.id;view.append(t);} }
